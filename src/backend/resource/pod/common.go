	...
	if !initializing {
		...
		for i := len(pod.Status.ContainerStatuses) - 1; i >= 0; i-- {
			...
			if container.State.Waiting != nil && container.State.Waiting.Reason != "" {
				...
				if container.State.Terminated.Signal != 0 {
					reason = fmt.Sprintf("Signa: %d", container.State.Terminated.Signal)
				} else {
		...
		// change pod status back to "Running" if there is at least one contaienr still reporting as "Running" status
