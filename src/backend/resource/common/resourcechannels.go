...
// ServiceListChannel is a listand error channels to Services.
...
// IngressListChannel is a list and error channels to Ingress.
...
// GetEventListChannel returns a pair of channels to an Event list and both must be read
// numReads times.
...
func GetReplicationControllerListChannel(client client.Interface, nsQuery *NamespaceQuery, numReads int) ReplicationControllerListChannel {
	...
	go func() {
		list, err := client.CoreV1().ReplicationControllers(nsQuery.ToRequestParam()).List(context.TODO(), api.ListEverything)
...
func GetDeploymentListChannel(client client.Interface, nsQuery *NamespaceQuery, numReads int) DeploymentListChannel {
	...
	go func() {
		list, err := client.AppsV1().Deployments(nsQuery.ToRequestParam()).List(context.TODO(), api.ListEverything)
...
// GetResourceQuotaListChannel returns a pair of channels to ResourceQuota list and errors that
// both must be read numReads times.
...
func GetHorizontalPodAutoscalerListChannel(client client.Interface, nsQuery *NamespaceQuery, numReads int) HorizontalPodAutoscalerListChannel {
	...
	go func() {
		list, err := client.AutoscalingV1().HorizontalPodAutoscalers(nsQuery.ToRequestParam()).List(context.TODO(), api.ListEverything)
		for i := 0; i < numReads; i++ {
			channel.List <- list
			channel.Error <- err
		}
