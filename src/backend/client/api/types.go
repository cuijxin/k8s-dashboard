...
const (
	// CsrfTokenSecretName is the resource information that are used as csrf token
	// storage. Can be accessible by multiple dashboard replicas.
	CsrfTokenSecretName = "kubernetes-dashboard-csrf"

	// CsrfTokenSecretData is the name of the data var that holds the csrf token
	// inside the secret.
	CsrfTokenSecretData = "csrf"
)

// ClientManger is responsible for initializing and creating clients to communicate
// with kubernetes apiserver on demand.
...
// ResourceVerber is responsible for performing generic CRUD operations on all
// supported resources.
...
// CanIResponse is used to as response to check whether or not user is allowed to
// access given endpoint.
type CanIResponse struct {
	Allowed bool `json:"allowed"`
}

// CsrfTokenManager is responsible for generating, reading and updating token
// stored in a secret.
