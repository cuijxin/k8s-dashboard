...
func (h *Handler) Install(ws *restful.WebService) {
	ws.Route(
		ws.GET("/plugin/config").To(h.handleConfig),
	)

	ws.Route(
		ws.GET("/plugin/{namespace}").To(h.handlePluginList).Writes(PluginList{}))

	ws.Route(
		ws.GET("/plugin/{namespace}/{pluginName}").To(h.servePluginSource))
}
