package jwe

import (
	"encoding/json"
	"time"

	jose "gopkg.in/square/go-jose.v2"

	authApi "github.com/cuijxin/k8s-dashboard/src/backend/auth/api"
	"github.com/cuijxin/k8s-dashboard/src/backend/errors"

	"k8s.io/client-go/tools/clientcmd/api"
)

// Implements TokenManager interface
type jweTokenManager struct {
	keyHolder KeyHolder
	tokenTTL  time.Duration
}

// AdditiaonalAuthData contains information required to validate token. It is
// integrity protected. For more information check:
// https://tools.ietf.org/html/rfc7516 (Chapter 2: Terminology)
type AdditionalAuthData map[Claim]string

// Claim represent token claims used in AAD header. For more information check:
// https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html#rfc.section.4
type Claim string

const (
	// Time format used when generating AAD header for token. Required to set token
	// creation/expiration time.
	timeFormat = time.RFC3339
	// IAT claim is part of token AAD header. It represents token "issued at" time.
	IAT Claim = "iat"
	// EXP claim is part of token AAD header. It represents token expiration time.
	EXP Claim = "exp"
)

// Generate and encrypt JWE token based on provided AuthInfo structure. AuthInfo
// will be embedded in a token payload and encrypted with autogenerated signing key.
func (t *jweTokenManager) Generate(authInfo api.AuthInfo) (string, error) {
	marshalledAuthInfo, err := json.Marshal(authInfo)
	if err != nil {
		return "", err
	}

	jweObject, err := t.getEncrypter().EncryptWithAuthData(marshalledAuthInfo, t.generateAAD())
	if err != nil {
		return "", err
	}

	return jweObject.FullSerialize(), nil
}

// Decrypt provides token and returns AuthInfo structure saved in a token payload.
func (t *jweTokenManager) Decrypt(jweToken string) (*api.AuthInfo, error) {
	jweTokenObject, err := t.validate(jweToken)
	if err != nil {
		return nil, err
	}

	decrypted, err := jweTokenObject.Decrypt(t.keyHolder.Key())
	if err == jose.ErrCryptoFailure {
		// Force key refresh and try to decrypt again
		t.keyHolder.Refresh()
		decrypted, err = jweTokenObject.Decrypt(t.keyHolder.Key())
	}

	if err != nil {
		return nil, err
	}

	authInfo := new(api.AuthInfo)
	err = json.Unmarshal(decrypted, authInfo)
	return authInfo, err
}

// Refresh implements token manager interface. See TokenManager for more information.
func (t *jweTokenManager) Refresh(jweToken string) (string, error) {
	if len(jweToken) == 0 {
		return "", errors.NewInvalid("Can not refresh token. No token provided.")
	}

	jweTokenObject, err := t.validate(jweToken)
	if err != nil {
		return "", err
	}

	decrypted, err := jweTokenObject.Decrypt(t.keyHolder.Key())
	if err != nil {
		return "", err
	}

	authInfo := new(api.AuthInfo)
	err = json.Unmarshal(decrypted, authInfo)
	if err != nil {
		return "", errors.NewInvalid("Token refresh error. Could not unmarshal token payload.")
	}

	return t.Generate(*authInfo)
}

// SetTokenTTL implements token manager interface. See TokenManager for more information.
func (t *jweTokenManager) SetTokenTTL(ttl time.Duration) {
	if ttl < 0 {
		ttl = 0
	}

	t.tokenTTL = ttl * time.Second
}

func (t *jweTokenManager) getEncrypter() jose.Encrypter {
	return t.keyHolder.Encrypter()
}

// Parses and validates provided token to check if it hasn't been manipulated with.
func (t *jweTokenManager) validate(jweToken string) (*jose.JSONWebEncryption, error) {
	jwe, err := jose.ParseEncrypted(jweToken)
	if err != nil {
		return nil, err
	}

	if t.tokenTTL > 0 {
		aad := AdditionalAuthData{}
		err = json.Unmarshal(jwe.GetAuthData(), &aad)
		if err != nil {
			return nil, errors.NewInvalid("Token validation error. Could not unmarshal AAD.")
		}

		if t.isExpired(aad[IAT], aad[EXP]) {
			return nil, errors.NewTokenExpired(errors.MsgTokenExpiredError)
		}
	}

	return jwe, nil
}

// Returns true if token has expired. In case time could not be parsed it might
// mean that token was tampered with and token will be marked as expired. This
// will force user to log in again.
func (t *jweTokenManager) isExpired(iatStr, expStr string) bool {
	iat, err := time.Parse(timeFormat, iatStr)
	if err != nil {
		return true
	}

	exp, err := time.Parse(timeFormat, expStr)
	if err != nil {
		return true
	}

	age := time.Now().Sub(iat.Local())
	return iat.Add(age).After(exp)
}

func (t *jweTokenManager) generateAAD() []byte {
	now := time.Now()
	aad := AdditionalAuthData{
		IAT: now.Format(timeFormat),
	}

	if t.tokenTTL > 0 {
		aad[EXP] = now.Add(t.tokenTTL).Format(timeFormat)
	}

	rawAAD, _ := json.Marshal(aad)
	return rawAAD
}

// Creates and returns default JWE token manager instance.
func NewJWETokenManager(holder KeyHolder) authApi.TokenManager {
	manager := &jweTokenManager{keyHolder: holder, tokenTTL: authApi.DefaultTokenTTL * time.Second}
	return manager
}
