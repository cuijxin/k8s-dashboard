...
// Below structures represent structure of kubeconfig file. They only contain fields required to
// gather data needed to log in user. It should support same options as defined in
// auth/api/types.go file. Currently: basic, token.
...
// Returns user info based on defined current. In case it is not found error is returned.
...
func (self *kubeConfigAuthenticator) getAuthInfo(info userInfo) (api.AuthInfo, error) {
	// If "token" is empty for the current "user" entry, fallback to the value of "auth-provideer.config.access-token".
	if len(info.Token) == 0 {
