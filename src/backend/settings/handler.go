...
func (self *SettingsHandler) Install(ws *restful.WebService) {
	...
	ws.Route(
		ws.PUT("/settings/global").
			...
			Reads(api.Settings{}).
			Writes(api.Settings{}))
	ws.Route(ws.GET("/settings/pinner").To(self.handleSettingsGetPinned))
	ws.Route(ws.PUT("/settings/pinner").To(self.handleSettingsSavePinned))
	ws.Route(ws.GET("/settings/pinner/cani").To(self.handleSettingsGlobalCanI))
	ws.Route(ws.DELETE("/settings/pinner/{kind}/{name}").To(self.handleSettingsDeletePinned))
	ws.Route(ws.DELETE("/settings/pinner/{kind}/{namespace}/{name}").To(self.handleSettingsDeletePinned))
}
...
func (self *SettingsHandler) handleSettingsGlobalSave(request *restful.Request, response *restful.Response) {
	...
	response.WriteHeaderAndEntity(http.StatusOK, settings)
}
