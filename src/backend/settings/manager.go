...
func (sm *SettingsManager) load(client kubernetes.Interface) (configMap *v1.ConfigMap, isDifferent bool) {
	configMap, err := client.CoreV1().ConfigMaps(args.Holder.GetNamespace()).Get(context.TODO(),
		api.SettingsConfigMapName, metav1.GetOptions{})
	if err != nil {
	...
	for key, value := range sm.rawSettings {
		if key == api.PinnedResourcesKey {
			...
			if err != nil {
				log.Printf("cannot unmarshal settigns key %s with %s value: %s", key, value, err.Error())
			} else {
				sm.settings[key] = *s
			}
		}
...
// GetGlobalSettings implements SettingsManager interface. Check if for more information.
